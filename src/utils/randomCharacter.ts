...
export function generateRandomCharacter(variant: GameVariant, forcedLevel?: number): CharacterData {
  ...
  // Racial bonuses
  const speciesBonuses: Partial<AbilityScores> = { ...race.abilityBonuses }
  if (subrace) {
    for (const [key, val] of Object.entries(subrace.abilityBonuses)) {
      const k = key as AbilityKey
      speciesBonuses[k] = (speciesBonuses[k] || 0) + (val || 0)
    }
  }
  ...
  if (race.abilityScoreChoice) {
    ...
    const alreadyBoosted = Object.keys(speciesBonuses) as AbilityKey[]
    ...
    for (const a of chosen) {
      speciesBonuses[a] = (speciesBonuses[a] || 0) + race.abilityScoreChoice.amount
    }
  }
  ...
  // Languages — #92 (PHB 2024): Common + 2 idiomas aleatorios.
  const allLanguages = getAvailableLanguages(variant)
  const candidates = allLanguages.filter(
    l => l.toLowerCase() !== 'common',
  )
  const extraLanguages = pickN(candidates, 2)
  const languages = ['Common', ...extraLanguages]

  // Calculate HP
  const conTotal = abilityScores.con + (speciesBonuses.con || 0)
  const conMod = modifier(conTotal)
  const maxHp = totalHp(cls.hitDie, conMod, level)
  const prof = proficiencyBonus(level)
  const strMod = modifier(abilityScores.str + (speciesBonuses.str || 0))
  const dexMod = modifier(abilityScores.dex + (speciesBonuses.dex || 0))
  ...
  if (cls.spellcasting) {
    ...
    const abilityMods = {
      str: modifier(abilityScores.str + (speciesBonuses.str || 0)),
      dex: modifier(abilityScores.dex + (speciesBonuses.dex || 0)),
      con: conMod,
      int: modifier(abilityScores.int + (speciesBonuses.int || 0)),
      wis: modifier(abilityScores.wis + (speciesBonuses.wis || 0)),
      cha: modifier(abilityScores.cha + (speciesBonuses.cha || 0)),
    }
