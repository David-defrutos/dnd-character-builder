...
function totalScore(ability: keyof CharacterData['abilityScores']): number {
  ...
  const cd = char.value
  const bonus = (cd.speciesBonuses?.[ability] ?? 0)
    + (cd.backgroundBonuses?.[ability] ?? 0)
    + (cd.asiBonuses?.[ability] ?? 0)
  return cd.abilityScores[ability] + bonus
}
      ...
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p class="text-stone-300 mt-1">
            {{ gt.speciesDisplay(char.race, char.subrace) }}
            &middot;
